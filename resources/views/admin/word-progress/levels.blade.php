                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($list as $lvl)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $lvl['category']->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $lvl['word_count'] }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">{{ $lvl['avg_score'] }}/10</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                @include('admin.partials.status-badge', ['status' => $lvl['status']])
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 whitespace-nowrap text-sm text-gray-500 text-center font-medium">
        ...
        @forelse($list as $lvl)
            <div class="bg-white shadow rounded-lg px-4 py-3">
                <div class="flex items-center justify-between gap-3 mb-2">
                    <div class="min-w-0">
                        <div class="font-bold text-gray-900 text-lg truncate">{{ $lvl['label'] ?? ('Level ' . $lvl['level']) }}</div>
                        <div class="text-xs text-gray-500">{{ $lvl['category']->name }}</div>
                    </div>
                    <div class="text-lg font-bold text-gray-900 flex-shrink-0">{{ $lvl['avg_score'] }}/10</div>
                </div>
                <div class="flex items-center gap-2 flex-wrap">
                    @include('admin.partials.status-badge', ['status' => $lvl['status']])
                    <span class="text-xs text-gray-500">{{ $lvl['word_count'] }} word{{ $lvl['word_count'] === 1 ? '' : 's' }} tried</span>
                </div>
            </div>
        @empty
